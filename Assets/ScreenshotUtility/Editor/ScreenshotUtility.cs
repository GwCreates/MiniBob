...
public sealed class ScreenshotUtility : EditorWindow, IHasCustomMenu
{
    ...
    private void RefreshCameras()
    {
        ...
        {
            //Auto select main camera if nothing was already selected
            if (cameras[i].CompareTag("MainCamera") && cameraID < 0) cameraID = i;
    ...
    void OnGUI()
    {
        if (cameras.Count == 0)
        {
            EditorGUILayout.HelpBox("No active camera's found (or they're hidden/have a render texture)", MessageType.Warning);
            return;
        }
        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
        {
            ...
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Target camera", GUILayout.MaxWidth(90f));
        ...
        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button(new GUIContent("...", "Browse"), EditorStyles.miniButtonLeft,GUILayout.Width(25f)))
                {
        ...
        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
        {
            EditorGUI.BeginDisabledGroup(SavePath == string.Empty);
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    if (GUILayout.Button(new GUIContent("- Capture -"), GUILayout.ExpandHeight(false), GUILayout.Height(30f)))
                    {
                        RenderScreenshot();
                    }
                }
    ...
    public void RenderScreenshot()
    {
        ...
        RenderTexture rt = new RenderTexture(ssWidth, ssHeight, 16, sourceCamera.allowHDR ? RenderTextureFormat.DefaultHDR : RenderTextureFormat.Default, RenderTextureReadWrite.sRGB);
        ...
        Texture2D screenShot = new Texture2D(ssWidth, ssHeight, transparency ? TextureFormat.ARGB32 : TextureFormat.RGB24, false, false);
    ...
    public static SettingsProvider ScreenshotSettings()
    {
        var provider = new SettingsProvider("Preferences/Screenshot", SettingsScope.User)
        {
            ...
            guiHandler = (searchContent) =>
            {
                ...
                ListenToPrintButton = EditorGUILayout.Toggle(new GUIContent("Print-screen button", "If the window is open and the scene-view is active, capture a screenshot whenever the print-screen keyboard button is pressed"), ListenToPrintButton);
            },
